...
private:
    ...
    std::string base_url_;                   // Twitter API基础URL
    std::string csrf_token_;                 // CSRF令牌
    std::string auth_token_;                 // 认证令牌
     ...
     */
    DMResult parseResponse(const cpr::Response& response, const std::string& user_id, const std::string& message) const;
...
public:
     ...
     */
    explicit Twitter(const std::string& cookies);
